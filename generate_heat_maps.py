	...
	try:
		...
		for county in counties:
			try:
				...
				# Load your custom city labels GeoJSON
				cities_gdf = gpd.read_file('inputs/geojson/ca_incorprated_cities.geojson')
				# Reproject to Web Mercator (EPSG:3857) to match your other layers
				cities_gdf = cities_gdf.to_crs(epsg=3857)
				# Get the county boundary as a single geometry
				county_shape_geom = county_shape.unary_union

				# Clip the cities GeoDataFrame to the county boundary
				cities_in_county = cities_gdf[cities_gdf.geometry.centroid.within(county_shape_geom)]

				for idx, row in cities_in_county.iterrows():
