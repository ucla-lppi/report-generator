"""
Generate a one-time statewide air pollution map for California (standalone, does not use map_utils.py).
"""
...
# File paths
geojson_path = 'inputs/geojson/ca_pm25_bins.geojson'
...
# Load county-level air pollution data
print(f"Loading county-level air pollution bins from {geojson_path}")
...
# Helper to blend color with white at given alpha
def blend_with_white(hex_color, alpha):
    rgb = mcolors.to_rgb(hex_color)
    blended = tuple([alpha * c + (1 - alpha) * 1.0 for c in rgb])
    return blended
# color_mapping = {
#     "Below State Average": "#d3ebe0",
#     "Near State Average": "#55947A",
#     "Above State Average": "#10462e",
# }
color_mapping = {
    "Below State Average": "#e7f4ee",
    "Near State Average": "#a2c4b6",
    "Above State Average": "#7c998c",
}
legend_mapping = {
    "Below State Average": "Below State Avg",
    "Near State Average": "Near State Avg",
    "Above State Average": "Above State Avg",
}
for label, color in color_mapping.items():
    mask = gdf['categorical_state'] == label
    if not gdf[mask].empty:
        # blended = blend_with_white(color, 0.8)
        gdf[mask].plot(ax=ax, color=color, edgecolor=color, linewidth=0.5, alpha=1.0, zorder=3)
...
MAJOR_CITIES = [
    ...
    "Fresno", "Sacramento", "Bakersfield", "Salinas","Eureka","Chico","Redding","Riverside","Coachella","Napa","Colusa","El Centro","South Lake Tahoe"
]
...
gdf.dissolve(by='County').boundary.plot(ax=ax, linewidth=1, edgecolor="#777A7873", alpha=0.45, zorder=10)
...
# ax.legend(handles=legend_elements, loc='upper right', title='PM2.5 Concentration Levels', frameon=True)

ax.set_axis_off()
plt.tight_layout()
plt.savefig(output_file, bbox_inches='tight', pad_inches=0.1, dpi=150)
plt.close()
print(f"Statewide air pollution map saved to {output_file}")
