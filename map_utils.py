...
map_configs = {
	...
	"air_pollution": {
		"value_field": "county_average_category",
		"color_mapping": {
			"below county average" : "#d3ebe0",
			"near county average": "#55947A",
			"above county average" : "#10462e",
		},
		"legend_mapping": {
			"below county average": "Below County Average",
			"near county average": "Near County Average",
			"above county average": "Above County Average",
		},
...
def generate_majority_tracts_map(
	...
	road_data_path=None,
	county_filter=None,
	show_legend=False   # newly added parameter
):
	try:
		...
		temp_output_geojson_path = 'output/temp_joined_data_'+str(map_type)+'.geojson'
		...
		for county in counties:
			try:
				...
				print('Column list is : ', joined_gdf.columns)
				...
				if county_gdf.empty:
					...
					continue
				neigh_col = f"Neighborhood_type{'_joined' if map_type=='air_pollution' else ''}"

				# keep one single nested filter
				latino_gdf = county_gdf[
					county_gdf[neigh_col].isin(
						["50+ Latino", "70+ Latino"] 
				...
				if legend_elements:
					...
					# )
					if show_legend:
						if map_type == "air_pollution":
							legend_title = "PM2.5 Concentration Levels"
						else:
							legend_title = "Number Extreme Heat Days"
						legend = ax.legend(
